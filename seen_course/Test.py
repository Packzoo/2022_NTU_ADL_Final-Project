...
def summary_csv_to_pd(args):
    ...
    test_input_path = args.test_data
...
def inference(args, user_features, item_features, x_train, y_train, x_test, all_item):
    ...
    # for test
    user_embedding = trainer.inference_embedding(model=model, mode="user", data_loader=test_dl, model_path=model_path)
    ...
    match_evaluation(args,user_embedding, item_embedding, x_test, all_item, raw_id_maps=raw_id_maps_path, topk=50)
...
def train(args, user_features, item_features, x_train, y_train, x_test, y_test, all_item):
    ...
    raw_id_maps_path = args.input_dir + "/raw_id_maps.npy"
    match_evaluation(args,user_embedding, item_embedding, x_test, all_item, raw_id_maps_path,topk=10)
