...
class MatchTrainer(object):
    ...
    def __init__(
        ...
        #self.evaluate_fn = mapk
        self.n_epoch = n_epoch
        self.early_stopper = EarlyStopper(patience=earlystop_patience)
        self.model_path = model_path

    def train_one_epoch(self, data_loader, loss_dict, epoch_i, log_interval=10):
        self.model.train()
        total_loss = 0
        ...
        for i, (x_dict, y) in enumerate(tk0):
            ...
            if (i + 1) % log_interval == 0:
                ...
                total_loss = 0
                loss_dict["epoch"].append(epoch_i+1)
    ...
    def fit(self, train_dataloader):
        ...
        for epoch_i in range(self.n_epoch):
            print('epoch:', epoch_i)
            self.train_one_epoch(train_dataloader,loss_dict,epoch_i)

            if self.scheduler is not None:
                if epoch_i % self.scheduler.step_size == 0:
                    print("Current lr : {}".format(self.optimizer.state_dict() \
                    ['param_groups'][0]['lr']))
    ...
    def validation_embedding(self, mode, data_loader):
        #inference
